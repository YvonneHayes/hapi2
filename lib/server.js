...
server.route({
  ...
  handler: (request, reply) => {
    ...
    new Books(request.payload).save()
      .then(data => {
        reply({posted: data});
      })
      .catch(err => {
...
server.route({
  ...
  path: '/books/{id}',
  handler: (request, reply) => {
    Books.findOneAndUpdate({author: request.params.id}, request.payload,
...
server.route({
  ...
  handler: (request, reply) => {
    ...
    Books.findOneAndRemove({author: request.params.id})
      .then(data => {
        if (data !== null) reply({deleted: data});
        else reply({error: 'This book does not exist'});
      })
      .catch(err => {
        reply({error:err});
      });
